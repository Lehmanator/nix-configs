  ...
  programs.nixvim.plugins.dap = {
    ...
    extensions = {
      ...
      dap-virtual-text = {
        ...
        allFrames = true; # Show virtual text for all stack frames, not just current. Default=false
        allReferences = true; # Show virtual text on all references of the variable, not only definitions. Default=false
        clearOnContinue = false; # Clear virtual text on continue (may cause flickering when stepping) Default=false
        commented = false; # Prefix virtual text w/ comment string. Default=false
        #displayCallback = null;
        highlightChangedVariables = true; # HL changed vals w/ `NvimDapVirtualTextChanged`, else always `NvimDapVirtualText`. Default=true
        highlightNewAsChanged = false; # HL new vars same as changed
        onlyFirstDefinition = true; # Only show virtual text at 1st definiton if multiple
