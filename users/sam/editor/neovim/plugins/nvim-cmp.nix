...
# TODO: Move borderStyle & theme logic to separate Nix module
{
  programs.nixvim.plugins = {
    ...
    cmp-cmdline-history.enable = true;       # Items from cmdline history (vim or shell?)
    ...
    cmp-nvim-lsp-document-symbol.enable = true;  # Neovim LSP symbols from documents
    ...
    cmp-pandoc-references.enable = config.programs.pandoc.enable;  # Pandoc references

    cmp-path.enable = true;                  # Filesystem paths
    cmp-rg.enable = true;                    # Ripgrep
    cmp-spell.enable = false;                # Spelling suggestions ?

    cmp-tabnine = {                          # Autocompletion from ML suggestion model
      enable = false;
      #extraOptions = { };
    };
