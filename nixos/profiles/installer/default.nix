{ config, lib, pkgs, modulesPath, ... }: {
  ...
  nix = {
    ...
    settings = { extra-features = [ "nix-channel" "nix-command" "flakes" ]; };
  };
  ...
  boot = {
    ...
    kernelPackages =
      pkgs.linuxPackages_latest; # TODO: Consider pkgs.linux_zen ?
    ...
    loader = {
      efi = {
        ...
        efiSysMountPoint =
          "/boot/efi"; # Default: `/boot`  # TODO: `XBOOTLDR` / `ESP` splitting. # TODO: Rewrite to `/efi`?
      };

      grub.enable =
        lib.mkForce false; # Disable GRUB & conditionally systemd-boot
      systemd-boot.enable = config.boot
        ? lanzaboote; # Only use systemd-boot if we don't have lanzaboote for SecureBoot
    };
  };
