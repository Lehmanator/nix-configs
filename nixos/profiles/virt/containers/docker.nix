...
}:
{
  ...
  virtualisation = {
    ...
    docker = {
      enable = false;
      enableNvidia = lib.mkDefault false; # Enable nvidia-docker wrapper, supporting NVIDIA GPUs inside docker containers.
      enableOnBoot = true;
      autoPrune = {
        enable = true;
        dates = "weekly";
      ...
      listenOptions = [ "/run/docker.sock" ]; # List of unix & TCP docker should listen to. Format follows ListenStream as described in systemd.socket(5)
      liveRestore = true; # Allow dockerd to be restarted w/o affecting running container. Option incompatible w/ docker swarm
      logDriver = "journald"; # *journald | none | json-file | syslog | gelf | fluentd | awslogs | splunk | etwlogs | gcplogs | local
      package = pkgs.docker;
      rootless = {
        enable = true; # Enables docker in rootless mode, a daemon that manages linux containers. To interact w/ daemon, one needs to set: `DOCKER_HOST=unix://$XDG_RUNTIME_DIR/docker.sock
        daemon.settings = {
          fixed-cidr-v6 = lib.mkIf config.networking.enableIPv6 "fd00::/80";
          ipv6 = config.networking.enableIPv6;
        };
