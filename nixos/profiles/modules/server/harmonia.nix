{ inputs, config, lib, pkgs, ... }:
{
  ...
  environment.persistence."/nix/persist".files = lib.mkIf config.services.harmonia.enable [ config.sops.secrets.harmonia-signing-key.path ];
  networking.firewall.allowedTCPPorts = lib.mkIf config.services.harmonia.enable [ 443 80 ];
