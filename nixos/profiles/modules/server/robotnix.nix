{ inputs, config, lib, pkgs, ... }:
{
  ...
  services.attestation-server = {
    ...
    email = {
      ...
      passwordFile = lib.mkDefault config.sops.secrets.email-dev-coverall-password.path;
  ...
  networking.firewall.allowedTCPPorts = lib.mkDefault [config.services.attestation-server.port];
