...
in
{
  ...
  environment.shellAliases = {
    ...
    nos-activate = "nixos-rebuild test";
    nos-activate-dry = "nixos-rebuild dry-activate";
    nos-boot = "nixos-rebuild boot";
    nos-build-dry = "nixos-rebuild dry-build";
    nos-build = "nixos-rebuild build";
    nos-rebuild = "nixos-rebuild build";
    nos-switch = "nixos-rebuild switch";
    nos-switch-dry = "nixos-rebuild dry-activate";
    nos-build-vm = "nixos-rebuild build-vm";
    nos-build-vmb = "nixos-rebuild build-vm-with-bootloader";
    nos-test = "nixos-rebuild test";
    nos-update = "nixos-rebuild switch --upgrade";
    nos-update-all = "nixos-rebuild switch --upgrade-all";

    nup = "nixos-rebuild switch --upgrade-all";
    Nup = "cd $NIXOS_FLAKE_SYSTEM && l && git status && nix flake update --commit-lock-file && sudo nixos-rebuild switch --upgrade-all";
