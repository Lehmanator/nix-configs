{ inputs, config, lib, pkgs, ... }:
{
  imports = [ ];
  boot.initrd.systemd = {
    enable = lib.mkDefault config.boot.loader.systemd.enable; #true;
    #package = pkgs.systemdStage1;
    dbus.enable = lib.mkDefault true;
    emergencyAccess = lib.mkDefault false; # false | true | null | <str> (hashed superuser password to lock emergency mode behind auth)
    ...
    additionalUpstreamUnits = [
      # Units to borrow from upstream for usage in initrd.
      "debug-shell.service"
    ...
    network = {
      ...
      wait-online = {
        # Enable `systemd-networkd-wait-online` service. Can timeout & fail if there arent network interfaces available to manage.
    ...
    repart = {
      # Grow & add partitions to table at boot time in initrd using `systemd-repart` (note: only works w/ GPT part tables)
