...
let
  ...
  getMachineConfigAttr =
    h: p:
    let
      path = if builtins.isList p then p else lib.splitString "." p;
    in
    lib.getAttrFromPath path inputs.self.nixosConfigurations.${h}.config;
...
# TODO: Configure self to work for other machines
{
  ...
  users.extraUsers.sftp = {
    ...
    # Iterate through all nixosConfigurations & add them to authorized users.
    openssh.authorizedPrincipals = lib.fold (
      item: acc:
      acc
      ++ [
        "sftp@${inputs.self.nixosConfigurations.${item}.config.networking.hostName}"
        "${user}@${inputs.self.nixosConfigurations.${item}.config.networking.hostName}"
        "${user}@${inputs.self.nixosConfigurations.${item}.config.networking.fqdn}"
      ]
