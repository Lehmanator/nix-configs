...
in
{
  ...
  programs.rust-motd = {
    ...
    order = [ "global" "banner" "weather" "uptime" "last_login" ] 
    ...
    settings = {
      ...
      global = let 
        get-caps = n: if builtins.isString then {
      ...
      filesystems = {
        ...
        home = "/home";
        nix-store = "/nix/store";
        #nix-persist = mkIf (config.environment.persistence."/nix/persist") "/nix/persist";
      };

      # Display last login for users in attrset.
      # TODO: Possible to use `config.users.users.extraUsers`, etc.?
      last_login = { root=4; ${user}=4; };
