...
in
{
  ...
  nix.settings = (if (osConfig ? home-manager && osConfig.home-manager.useGlobalPkgs)
    then osConfig.nix.settings
    else defaults
  ) // {
    builders-use-substitutes = mkDefault true; # Allow builders to use binary caches
    substitute = mkDefault true;
