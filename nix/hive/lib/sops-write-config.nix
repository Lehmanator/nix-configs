...
#  ...
#}:
filename: config: {user ? "sam", ...} @ funArgs: ((inputs.nixpkgs.formats.yaml {}).generate filename {
  ...
  creation_rules = let
    configuration-types = ["common" "darwin" "droid" "liminix" "linux" "nixos" "robotnix" "wsl"];
    # Illegal paths:
    #
    ...
    configurations-regex = "(${inputs.nixpkgs.lib.lists.concatStringsSep "|" configuration-types})";
    file-extensions = [
      "asc"
      "auth.*"
      "bin"
