...
inputs.nixpkgs.lib.makeOverridable ({
  ...
  let
    selfSystem = inputs.nixpkgs.lib.nixosSystem {
          ...
          system.build = let
            installerConfiguration = lehmanatorSystem {
              ...
              modules = [
                {
                  nixpkgs = {
                    hostPlatform = selfSystem.config.nixpkgs.hostPlatform;
                    buildPlatform = selfSystem.config.nixpkgs.buildPlatform;
                  };
                }
                (inputs.nixpkgs.lib.optionalAttrs (selfSystem.config.system.build ? diskoScript) {
                ...
                })
                {
                  ...
                  } // installDiskoScript = inputs.nixpkgs.lib.optionalAttrs (selfSystem.config.system.build ? diskoScript) selfSystem.config.system.build.diskoScript;
                }
      ...
      in baseModules ++ modules ++ installerModule;
    };
  in selfSystem);
