...
}:
{
  ...
  services.wgautomesh = {
    ...
    gossipSecretFile = "/etc/wireguard/wgautomesh-gossip.secret"; # File for gossip secret, shared secret key for gossip encryption. Required if enableGossipEncryption=true. File may contain any arbitrary-length utf8 str. Generate: `openssl rand -base64 32`
    logLevel = "info";
