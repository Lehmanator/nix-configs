{
  ...
  imports = [
    inputs.flake-gemini.nixosModules.duckling-proxy
    inputs.flake-gemini.nixosModules.kineto
    inputs.flake-gemini.nixosModules.molly-brown
  ];
  ...
  services = {
    duckling-proxy = {
      ...
      serverCert =
        lib.mkDefault config.sops.secrets.duckling-proxy-server-cert.path;
      serverKey =
        lib.mkDefault config.sops.secrets.duckling-proxy-server-key.path;
    };
    kineto = {
      enable = lib.mkDefault false;
