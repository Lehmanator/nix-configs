  ...
  in
{
  ...
  services.tailscale = {
    ...
    interfaceName = if cfg.userspace then "userspace-networking" else "tailscale0";
    openFirewall = true;
    permitCertUid = user;
    useRoutingFeatures = if cfg.client then "both" else "server"; # none|client|server|both
  };
