...
}:
{
  ...
  documentation = {
    ...
    man = {
      ...
      generateCaches = true;
      man-db.enable = false;
      mandoc.enable = true; # Note: man-db & mandoc cannot both be enabled simultaneously
    };
    nixos = {
      enable = true; # Include dev docs if documentation.man.enable=true (`devman`,`devinfo`,`devdoc` outputs)
      options.splitBuild = lib.mkForce true; # Split opt docs into cacheable/not. Faster build. Some user modules may break. Default=true
